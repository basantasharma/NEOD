...
<div class="col-6 mt-3 ms-3">
    ...
    <form action="{{route('admin.postPages')}}" method="POST" class="p-1" enctype="multipart/form-data">
        @csrf
        <div>
            <label for="title" class=" mt-2">Title</label>
            <input class="form-control mb-2 mt-2" type="text" name="title" id="title" required>
...
<textarea name="description" id="description" cols="30" class="form-control" rows="10" required></textarea>
<div class="mt-1" >
    @error('description')
      <span class="text-danger">{{$message}}</span>
  @enderror
